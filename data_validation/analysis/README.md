...
For every directed consecutive-stop pair on every subway line, computes mean/stdev/CV from `total`, then ranks by stdev and by CV (is the spread small relative to the mean?), and separately checks for a real rush/off-peak pattern via an hourly breakdown (does the mean hide a time-of-day shift a static weight would wash out?). Every flagged edge also gets `total` split into `door` (`departure_a - arrival_a`, dwell at the boarding platform) and `sw` (`arrival_b - departure_a`, time actually spent moving) as a diagnostic for *why* it's noisy — the edge weight itself stays `total`. Ends with a verdict: how many edges pass, how many need attention, and which test (CV, hourly range, or both) flagged each one.

Because it reads the post-shared-platform-duplication `stop_times_shared.txt`, it uses `subway_route_names_stop_ids_artificial` (`scripts/basics.py`) rather than the original `subway_route_names_stop_ids` — unlike the other two scripts on this page, which read the pre-duplication `stop_times_doors.txt`.
